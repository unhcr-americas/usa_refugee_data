...
vignette: >
  %\VignetteIndexEntry{Accepted Refugees United States}
...
pacman::p_load(tidyverse, readxl, sjmisc, urbnmapr, tabulizer, viridis, magrittr, countrycode)
...
year_dat <- tibble(year = c(2005, 2009, 2013, 2017), 
                   label = c("Bush II", "Obama I", "Obama II", "Trump I"))

qualitative <- c('#a6cee3','#1f78b4','#b2df8a','#33a02c','#fb9a99','#e31a1c','#fdbf6f','#ff7f00','#cab2d6','#6a3d9a')

n_refugee_total <- refugee_dat %>% 
                    use_series(n) %>% 
                    sum
...
gg_relig <- relig_refugee  %>% 
  ...
  labs(x = "", y = "Number of Refugees\n", 
       ...
       caption = glue("Data: Department of State, Office of Admissions - Refugee Processing Center. Total accepted Refugees in Timerange: {scales::comma(n_refugee_total)}; Visualization: @favstats"))   +
  guides(color = F)
...
city_stats_by_year %>% 
  ...
  top_n(25) %>% 
  ungroup() %>% 
  mutate(city_state = forcats::fct_reorder(city_state, n)) %>% 
  ggplot(aes(city_state, n)) +
  geom_col() +
  coord_flip()  +
  theme_minimal() +
  ...
  labs(x = "", y = "Number of Refugees\n", 
       ...
       caption = glue("Data: Department of State, Office of Admissions - Refugee Processing Center. Total accepted Refugees in Timerange: {scales::comma(n_refugee_total)}; Visualization: @favstats")) +
  guides(color = F) +
  geom_text(aes(label = n), color = "white", nudge_y = -1000)
