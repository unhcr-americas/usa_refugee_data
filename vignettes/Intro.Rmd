...
refugee_dat <- read_excel(glue::glue("{mainDirroot}/data/refugee_dat.xls")) %>% 
  drop_na(X__1) %>% 
  rename(cntry = X__1) %>% 
  select(-Religion, -X__2, -X__3, - Total) %>% 
  filter(!(str_detect(cntry, "Total|Data"))) %>% 
  gather(year, n, -cntry) %>% 
  mutate(year = str_replace(year, "CY ", "") %>% 
  as.numeric)
...
```{r}
year_dat <- tibble(year = c(2005, 2009, 2013, 2017, 2021), 
                   label = c("Bush II", "Obama I", "Obama II", "Trump I", "Biden"))
